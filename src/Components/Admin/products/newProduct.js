...
export default class NewProduct extends Component{
    ...
    handleAddPictures = () => {
        if(this.state.pictureUrl.length > 1) {
            ...
            images.push(this.state.pictureUrl)
    ...
    handleSubmit = (event) => {
        ...
        if(this.state.brand.length > 40) {
            this.setState({brandError: "40 caractères maximum"})
            canSend = false;
        }

        if(this.state.colors.length > 40) {
        ...
        for (let i=0; i < this.state.pictures.length; i++) {
            if(!this.isValidHttpUrl(this.state.pictures[i])) {
                urlOk = false
                urlNbr = i+1;
            }
        }
                ...
                <div className={"container"}>
                    <div className={"row justify-content-center mt-5"}>
                        <div className={"col-6"}>
                            ...
                            <form className="NewProduct_container global_bgColor--whiteSmoke container font_montserrat d-flex flex-column align-items-center pb-5 mb-5" onSubmit={this.handleSubmit}>
                                <div className={"col-10"}>
                                    ...
                                    { this.state.pictures && this.state.pictures.length > 0?
                                        <div className={"d-flex justify-content-around"}>
                                            { this.state.pictures.map((picture, index) => {
                                                return <img className={"little--picture"} src={picture} key={index} alt={index}/>
                                            }) }
                                        </div>:null
                                    }
                                    ...
                                    <div className="row justify-content-center mt-3">
                                        <input type="submit" className={"btn mt-4 col-5 global_bgColor--charcoal global_fontColor--whiteSmoke NewProduct_create_btn"} value="Envoyer" />
                                    </div>
                                </div>
                            </form>
                        </div>
